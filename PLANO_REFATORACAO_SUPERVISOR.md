...
## Projeto Oráculo — `router/` + `agents/` + `tools/`

> **Status:** documento de planejamento. Nenhuma alteração de código foi feita nesta fase — apenas varredura read-only de `src/`, `docker-compose.yml` e `arquitetura_oraculo.md`.

---

## 0. Decisão Arquitetural Prévia: onde ficam `router/`, `agents/`, `tools/`?
...
- `router/` vira o único ponto de entrada de decisão "qual agente chamar" — precisa ser importável por `application/tasks/process_message_task.py` sem depender de nada de `agents/` internamente (só de contratos/enum de nomes de agente), o que é mais simples de garantir como pacote de topo-nível do que soterrado em `application/routing/`.
- `tools/` de topo nível deixa claro, visualmente e por convenção de import (`from src.tools.sigaa import ...`), que **tools não têm lógica de decisão** — são a nova "infrastructure fina" convocável por qualquer agente. Isso não elimina `infrastructure/` (adapters de Evolution API, Postgres, Redis client continuam lá como portas técnicas de baixo nível); `tools/` é a camada de **funções de negócio atômicas e burras** que embrulham 1+ adapters de infra para uma operação específica (ex: `tools/sigaa/scrape_historico.py` usa Playwright + `infrastructure/scraping` por baixo).
...
src/
  ...
  agents/
    academic_rag/     # RAG acadêmico + synthesis
    sigaa/             # especialista SIGAA (elegibilidade, formatação, orquestra tools de scraping)
    chat/              # saudação, boas-vindas, funil de cadastro
    action/            # GLPI/ações administrativas
  tools/
...
`application/` não desaparece: continua sendo o "sistema nervoso" que conecta webhook → router → agents → tools → resposta, mas deixa de conter lógica de negócio ou chamadas LLM diretas.
...
| `src/application/workers/registry.py` | Autodiscovery via `pkgutil` + decorator, mecanismo de plugin limpo |
...
src/router/
  __init__.py
  supervisor.py        # decide qual AGENT_NAME chamar; 5 camadas (regex, heurística, regex seeded, KNN Redis, fallback LLM)
...
- `src/application/routing/semantic_router.py` → `src/router/supervisor.py` (refatorado: chamada Gemini extraída para `llm_fallback.py`; import de `sigaa_use_cases` removido — router não deve importar domain use-case, deve só retornar um nome de agente)
...
- `src/domain/tools/tool_registry.py` → **DELETAR** as `description=` de tool (fonte de verdade duplicada); qualquer schema de tool reaproveitável migra para `tools/registry.py` (ver 2.5), mas sem texto de intenção — só assinatura/contrato de IO
...
4. **IO Redis cru** (get/setex/delete/xadd/xread espalhado) → extraído para `tools/persistence/redis_state.py`, uma tool burra de "gerenciamento de estado conversacional" (set/get/expire de estado de sessão) consumida tanto pelo dispatcher quanto pelos agentes que precisam de HITL.
...
### 2.3 SIGAA: separar `sigaa_agent.py` (966 linhas) em tools puras vs agente especialista

- **`agents/sigaa/`**:
  - `service.py` — orquestra o fluxo: decide QUAL scraping rodar, calcula elegibilidade de matrícula/próximo semestre (hoje duplicado entre `worker_sigaa.py` e `sigaa_agent.py` — consolidar num único lugar aqui), formata mensagens de resposta ao usuário.
...
### 2.4 Agente "Synthesis/RAG" coeso

Consolidar `synthesis_service.py`, `worker_synthesis.py`, `planner.py`, `worker_memory_manager.py`, e a parte de decisão de `rag_search_service.py` num único `agents/academic_rag/`:

```
src/agents/academic_rag/
  service.py          # orquestra: recebe pergunta → decide reescrita de query → retrieval → synthesis → resposta
...
- `infrastructure/services/rag_search_service.py` (611 linhas) → dividido: a parte "decide o que buscar e como combinar" (RRF/fallback step-back, `buscar()`) vira `agents/academic_rag/service.py`; a parte de IO pura (chamadas KNN Redis) vira `tools/rag/retrieval.py`.
- `worker_synthesis.py` (408 linhas) → reduzido a um wrapper Celery fino chamando `agents/academic_rag/service.py`, eliminando a chamada Gemini duplicada hoje existente no worker.
...
- Admin overrides do Redis (`admin:gemini_blocked`, `admin:system_prompt`), hoje lidos dentro de `synthesis_service.py` → migram para `tools/persistence/admin_config.py` (tool burra de leitura de config), injetada em `agents/academic_rag/service.py` via parâmetro, não lida diretamente pela camada de prompt.

### 2.5 Consolidação do registro de tools

- `domain/tools/tool_registry.py` (morto, `StructuredTool` factory) vs `domain/tools/crud_tools.py` (dict `_TOOL_REGISTRY` + decorator `@tool`, também paralelo) → consolidar num único `tools/registry.py`, adotando o mecanismo de **decorator + dict** de `crud_tools.py` por ser mais simples e já ter um padrão real de uso, mas sem embutir `description=` de intenção (essa responsabilidade já migrou para `router/contracts.py`, ver 2.1). O registry serve só para permitir que agentes descubram tools disponíveis por nome, análogo ao `WorkerRegistry` do Celery (autodiscovery já citado como limpo — reaproveitar o mesmo padrão de `pkgutil` + decorator).
- `domain/tools/admin_tools.py` (`asyncio.run()` anti-pattern, toca ORM direto) → dividir: a query ORM pura (`Pessoa`, `select()`) vira `tools/persistence/admin_repository.py` (função async nativa, sem `asyncio.run()`); qualquer decisão de quando/por que chamar fica em `agents/chat/` ou num futuro `agents/admin/` se justificado por volume.
- `domain/tools/gmail_tool.py` (bom exemplo de tool burra) → migra quase inalterado para `tools/messaging/gmail_tool.py`, mantendo o padrão `StructuredTool` fino sobre `gmail_service` de infra.

### 2.6 `registration_funnel.py` e `worker_action.py`

- `application/routing/registration_funnel.py` (SQL cru + chamada direta a `EvolutionAdapter`) → a máquina de estados de cadastro é lógica de negócio de "boas-vindas/onboarding", então migra para `agents/chat/registration.py`; o SQL cru migra para `tools/persistence/registration_repository.py`; a chamada a `EvolutionAdapter` (envio de mensagem) passa a ser feita através de `tools/messaging/evolution_tool.py`, uma tool fina que embrulha o adapter de infra já existente (`infrastructure/adapters/evolution_adapter.py` permanece como está — é o adapter técnico; a tool é a camada que os agentes chamam).
- `application/workers/worker_action.py` (UPDATE SQL cru, stub GLPI) → o SQL vira `tools/persistence/action_repository.py`; a decisão de qual ação tomar (se/quando abrir chamado GLPI) migra para um `agents/action/service.py` (ou fica dentro de `agents/chat/` se o volume de "ações administrativas" for pequeno — decisão a validar durante a Fase 6, não bloqueia o roadmap).

### 2.7 Tabela-resumo (arquivo → destino)

| Arquivo Atual | Destino | Ação |
|---|---|---|
| `application/chain/cognitive_os.py` | `router/supervisor.py` + `application/runtime/dispatcher.py` + `agents/sigaa/auth_flow.py` + `tools/persistence/redis_state.py` | Decompor |
...
| `application/chain/planner.py` | `agents/academic_rag/planning.py` + `router/contracts.py` (whitelist) | Decompor |
| `application/routing/registration_funnel.py` | `agents/chat/registration.py` + `tools/persistence/registration_repository.py` | Decompor |
| `application/chain/guardrails.py` | `application/runtime/guardrails.py` | Mover (inalterado, bom modelo) |
| `application/chain/reranker.py` | `tools/rag/reranker.py` | Mover + remover probe de rede embutida |
| `domain/tools/tool_registry.py` | `tools/registry.py` (mecanismo) | Consolidar, deletar descriptions de intenção |
| `domain/tools/crud_tools.py` | `tools/registry.py` (mecanismo escolhido) | Consolidar |
| `domain/tools/admin_tools.py` | `tools/persistence/admin_repository.py` | Dividir, remover `asyncio.run()` |
| `domain/tools/gmail_tool.py` | `tools/messaging/gmail_tool.py` | Mover (inalterado) |
| `infrastructure/services/rag_search_service.py` | `agents/academic_rag/service.py` (decisão) + `tools/rag/retrieval.py` (IO) | Decompor |
| `infrastructure/services/synthesis_service.py` | `agents/academic_rag/synthesis.py` + `agents/academic_rag/prompts.py` | Decompor |
| `application/workers/worker_sigaa.py` | `application/workers/worker_sigaa.py` (fino) chamando `agents/sigaa/service.py` | Emagrecer |
| `infrastructure/scraping/implementations/sigaa_agent.py` | `tools/sigaa/scrape_*.py` (IO) + `agents/sigaa/service.py` + `agents/sigaa/eligibility.py` (decisão) | Decompor |
| `application/workers/worker_synthesis.py` | `application/workers/worker_synthesis.py` (fino) chamando `agents/academic_rag/service.py` | Emagrecer |
| `application/workers/worker_memory_manager.py` | `agents/academic_rag/memory_summarizer.py` + worker fino | Decompor |
| `application/workers/worker_action.py` | `agents/action/service.py` + `tools/persistence/action_repository.py` | Decompor |
...
### Fase 2 — Criar `router/` e migrar o roteamento (Supervisor primeiro)
- **Objetivo**: extrair `router/` como pacote de topo-nível, com `supervisor.py`, `llm_fallback.py`, `gatekeeper.py`, `contracts.py`, portando os padrões de Pydantic/DI dos routers mortos. `application/routing/semantic_router.py` e `message_router.py` passam a ser **shims de compatibilidade** (thin wrapper re-exportando do novo `router/`) por 1 release, para não quebrar imports não mapeados.
- **Arquivos tocados**: novo `src/router/*`; `application/routing/semantic_router.py`, `application/routing/message_router.py`, `application/routing/llm_orchestrator.py` viram shims; `application/tasks/process_message_task.py` e `application/chain/cognitive_os.py` atualizam imports para `router.supervisor`.
...
- **Objetivo**: eliminar o God Object central. Criar `application/runtime/dispatcher.py`, `tools/persistence/redis_state.py`, e mover HITL/validação CPF-senha para `agents/sigaa/auth_flow.py` (mesmo que `agents/sigaa/` ainda não exista completo, criar o esqueleto mínimo aqui).
- **Arquivos tocados**: `application/chain/cognitive_os.py` (esvaziado/deletado ao final da fase), `application/tasks/process_message_task.py` (passa a chamar `router.supervisor` + `application.runtime.dispatcher` diretamente), novo `agents/sigaa/auth_flow.py`, novo `tools/persistence/redis_state.py`.
...
### Fase 4 — Extrair `agents/academic_rag/` e `tools/rag/`
- **Objetivo**: consolidar RAG/Synthesis/Planner/Memory num agente coeso.
- **Arquivos tocados**: `infrastructure/services/rag_search_service.py`, `infrastructure/services/synthesis_service.py`, `application/chain/planner.py`, `application/workers/worker_memory_manager.py`, `application/workers/worker_synthesis.py` (emagrecido), `application/chain/reranker.py` → `tools/rag/reranker.py`.
...
### Fase 5 — Extrair `agents/sigaa/` e `tools/sigaa/`
- **Objetivo**: maior arquivo do projeto (`sigaa_agent.py`, 966 linhas) decomposto em tools de scraping puras + agente de decisão/elegibilidade.
- **Arquivos tocados**: `infrastructure/scraping/implementations/sigaa_agent.py` (deletado ao final), `application/workers/worker_sigaa.py` (emagrecido), novo `agents/sigaa/service.py`, `agents/sigaa/eligibility.py`, `tools/sigaa/scrape_*.py`.
...
### Fase 6 — Consolidar `tools/registry.py`, `tools/messaging/`, `tools/persistence/`, `agents/chat/`, `agents/action/`
- **Objetivo**: fechar as últimas duplicações (`crud_tools.py` vs `tool_registry.py`, `registration_funnel.py`, `worker_action.py`, `admin_tools.py`).
...
- **Critério de sucesso**: testes de cadastro/registro passam; nenhuma tool duplicada registrada duas vezes (checar log de startup do `tools/registry.py` autodiscovery); smoke test de fluxo completo de cadastro de novo usuário via WhatsApp (staging).
- **Reversível**: sim.

### Fase 7 — Limpeza final e remoção de shims
- **Objetivo**: remover os shims de compatibilidade criados na Fase 2 (`application/routing/*.py` como re-exports), remover `domain/tools/` e `infrastructure/services/{rag_search_service,synthesis_service}.py` originais, atualizar `arquitetura_oraculo.md` para refletir o novo desenho de 3 camadas (router/agents/tools) substituindo a descrição de "5 camadas cognitivas" antiga.
