"""
REPL manual do grafo de orquestração (`src/application/orchestration/`).
Rodar da raiz do repo, com a venv local ativa:

    python -m scripts.graph_repl

Simula uma conversa multi-turno (igual WhatsApp) contra o StateGraph,
reaproveitando Redis/Postgres/Gemini já configurados no .env (os mesmos
containers do `main` — não sobe Docker novo). Testa dois casos:
