...
# `OSResult` foi extraído para `runtime/contracts.py` (Fase 2, Parte C) — os
# agentes que consomem o contrato deixam de importar do dispatcher legado.
# Re-exportado aqui para os call sites existentes.
from src.application.runtime.contracts import OSResult  # noqa: E402,F401
...
# Ingestão de áudio (STT) + gatilho "responda em áudio" foram extraídos para
# `runtime/audio_intake.py` (Fase 2, Parte C) — remove a dependência que
# `dispatcher_langgraph.py` e `process_message_task.py` tinham deste módulo.
# Re-exportado aqui para os call sites existentes.
from src.application.runtime.audio_intake import (  # noqa: E402,F401
    _quer_resposta_em_audio,
    _remover_pedido_audio,
    _transcrever_audio_recebido,
)
