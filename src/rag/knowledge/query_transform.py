"""
src/rag/knowledge/query_transform.py
===================================================
...
Havia um `src/rag/query_transform.py` homônimo, de um pipeline de RAG mais
antigo que nunca esteve no caminho quente — **removido em 2026-09-09** junto
com a cadeia de use-cases que o exercitava (item A8c). Este aqui é o único
que existe.
"""
...
class QueryTransformService:
    ...
    async def transformar_com_flash(
        ...
        **Desligada por padrão no v1 (item B6).** O gatilho abaixo (pronome OU
        query curta) casa com quase toda pergunta real de WhatsApp — "como
        emito minha declaração?" tem "como", "qual o telefone da biblioteca?"
        tem "qual" — então isto não era uma otimização ocasional: era uma
        SEGUNDA chamada de LLM em praticamente toda resposta, além da síntese.
        Com o menu, a rota e o `doc_type` já vêm da tecla apertada, que era a
        maior parte do contexto que essa reescrita tentava adivinhar.

        As estratégias LOCAIS (nome próprio, sinônimos, step-back) continuam
        ligadas — são regex, custam zero e é delas que vem a maior parte do
        ganho de recall. Religar isto é mudar `FEATURE_QUERY_TRANSFORM_LLM`.
        """
        from src.infrastructure.settings import settings

        if not settings.FEATURE_QUERY_TRANSFORM_LLM:
            return query
