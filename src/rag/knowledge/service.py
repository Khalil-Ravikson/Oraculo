...
from src.rag.knowledge.query_transform import QueryTransformService
...
class RAGSearchService:
    ...
    async def buscar(
        ...
        try:
            ...
            # 7. Filtragem por doc_type
            #
            # `taxonomia_estrita` existe porque o comportamento certo depende de
            # QUEM escolheu o `doc_type`:
            #
            #   · Escolheu um classificador (heurística, palpite sobre o texto):
            #     errar a taxonomia é comum, então cair de volta na busca ampla
            #     salva a resposta. É o comportamento histórico.
            #   · Escolheu o USUÁRIO, apertando uma tecla do menu: a taxonomia
            #     não é palpite, é instrução. Anular o filtro aqui responderia
            #     uma pergunta sobre telefone com texto da wiki, e ainda pagaria
            #     uma síntese para produzir uma recusa — foi exatamente o que
            #     aconteceu em 2026-09-10, 1747 tokens para dizer "não encontrei".
            if doc_type and doc_type != "geral" and not source_filter:
                filtrados = [r for r in resultados if r.get("doc_type") == doc_type]
                if filtrados:
                    resultados = filtrados
                elif taxonomia_estrita:
                    logger.info(
                        "🔎 [RAG] nada em doc_type=%s e taxonomia estrita — "
                        "devolvendo vazio sem chamar o LLM (query=%.40s)",
                        doc_type, query,
                    )
                    return ToolResult.success(
                        message="Não encontrei essa informação nos documentos da UEMA.",
                        data={"chunks": [], "found": False, "doc_type": doc_type},
                    )
                else:
